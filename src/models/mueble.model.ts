...
export const buscarMueblePorId = (id: number): Promise<IResponse> => {
  return new Promise((resolve, reject) => {
    dbConn.query(`Select * from mueble where idMueble =${id}`, (err, res: IMueble) => {
      let response: IResponse = {} as IResponse;
      if (err) {
        response = { ...response, error: 400, mensaje: 'error' }
        reject(response);
...
export const buscarUsuariosAsignadosMueble = (id: number): Promise<IResponse> => {
  return new Promise((resolve, reject) => {
    dbConn.query(`
...
export const buscarMateriasPrimasAsignadosMueble = (id: number): Promise<IResponse> => {
  return new Promise((resolve, reject) => {
    dbConn.query(`
...
export const insertarMuebleListados = (listados: any) => {
  return new Promise(async (resolve, _reject) => {
    ...
    for (let i = 0; i < listados.materiaPrima.length; i++) {
      await insertarMuebleMateriaPrima(listados.materiaPrima[i], listados.idMueble);
    }
...
export const insertarMuebleMateriaPrima = (muebleMateriaPrima: any, idMueble: number) => {
